  ...
  Future<Result<dynamic>> get(Uri uri) async {
    try {
      ...
      if (response.statusCode >= 200 && response.statusCode < 300) {
        ...
        return Failure(ServerError(response.statusCode, 'El servidor respondio con codigo ${response.statusCode}'));
      }
    } on SocketException {
      return Failure(const NetworkError('No se pudo establecer conexion de red. Revisa tu conexion.'));
    } on TimeoutException {
      return Failure(const NetworkError('La conexion supero el tiempo de espera. Reintenta de nuevo.'));
    } catch (e) {
...
class PantallaPaso5 extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    return Scaffold(
      appBar: AppBar(
        title: const Text('Paso 5 - Arquitectura Completa MP'),
        ...
        loading: () => const Center(
          child: Column(
            ...
            children: [
              ...
              SizedBox(height: 16),
              Text(
                'Cargando desde arquitectura limpia MP...',
