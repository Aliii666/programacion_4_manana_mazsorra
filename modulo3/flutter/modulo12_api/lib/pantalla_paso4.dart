...
import 'pantalla_paso3.dart'; // Reutilizamos Producto y ProductoDto de la pantalla anterior
...
class NetworkError extends ApiError {
  const NetworkError([String msg = 'Sin conexión a Internet. Verifica tu red.']) : super(msg);
}
...
class UnknownError extends ApiError {
  ...
  const UnknownError(this.originError, [String msg = 'Ocurrió un error inesperado.']) : super(msg);
}
...
class _PantallaPaso4State extends State<PantallaPaso4> {
  ...
  Future<Result<List<Producto>>> _fetchProducts(String urlString) async {
    try {
      ...
      if (res.statusCode == 200) {
        ...
        return Failure(ServerError(res.statusCode, 'El servidor respondió con código ${res.statusCode}'));
      }
    } on SocketException {
      return Failure(const NetworkError());
    } catch (e) {
      return Failure(UnknownError(e, 'Excepción capturada: $e'));
    }
  }
    ...
    if (_isLoading) {
      return const Column(
        ...
        children: [
          ...
          SizedBox(height: 16),
          Text('Cargando petición...', style: TextStyle(color: Colors.grey)),
        ],
      );
    }
    ...
    if (res == null) {
      return const Column(
        ...
        children: [
          ...
          SizedBox(height: 12),
          Text(
            'Presiona uno de los botones de arriba para iniciar una petición.',
  ...
  Widget _buildSuccessList(List<Producto> productos) {
    if (productos.isEmpty) {
      return const Text('Petición exitosa, pero no se devolvieron productos.');
    }
      ...
      children: [
        Row(
          children: [
            ...
            Text(
              'Petición Exitosa (${productos.length} items)',
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          const SizedBox(height: 16),
          Text(
            'Categoría del Error: ${error.runtimeType}',
            style: TextStyle(
              fontSize: 11,
